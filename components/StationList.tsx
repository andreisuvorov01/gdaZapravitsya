...
import { bestPrice, QUEUE_LABELS, type FuelType, type StationStatus } from "@/lib/types";
...
// Единый порядок/подписи вкладок списка — общие для десктопного ListView и
  ...
  if (sorted.length === 0) {
    return (
      ...
      >
        ...
        <p className="text-base font-semibold text-white">{EMPTY_TEXT[mode].title}</p>
        ...
        <li key={s.id}>
          ...
          >
            <span className="relative shrink-0">
              ...
              <span
                className="absolute -bottom-0.5 -right-0.5 h-3.5 w-3.5 rounded-full ring-2 ring-[#141a21]"
                style={{
              ...
              {s.conflicting && (
                <span
                  className="station-conflict-dot"
                  title="Данные расходятся"
                  aria-hidden
                />
              )}
              {cheapestOnly && fuelType !== "all" && (
